...
try:
    ...
    CharacterGroupName = sys.argv[4]
    SkeletonGroupName = sys.argv[5]
    ModelGroupName = sys.argv[6]
    ...
    if isinstance(e, IndexError):
        print("Parameter is missing .... <sourceFBXFile> <FileDir> <FileName> <CharacterGroupName> <SkeletonGroupName> <ModelGroupName>")
    else:
        print(str(e))
    sys.exit()
...
def MoveGroupToOutLineAndExport():
    CharacterChilds = cmds.listRelatives(CharacterGroupName)
...
def exportFBX(exportFileName): # , min_time, max_time
    ...
    # Version
    mel.eval("FBXExportFileVersion -v FBX201800")                   # FBX 文件格式版本
