...
import sys
try:
    mayapy_path = sys.argv[1]
    sourceFBXFile = sys.argv[2]
    saveFileDir = sys.argv[3]
    saveFileName = sys.argv[4]
    print(mayapy_path,sourceFBXFile,saveFileDir,saveFileName)
    ...
    if not os.path.isdir(mayapy_path):
        raise Exception(mayapy_path,"MayaPython dir is not..")
except Exception as e:
    if isinstance(e, IndexError):
        print("Parameter is missing . ... <mayapy_path> <sourceFBXFile> <FileDir> <FileName>")
    else:
        print(str(e))
    sys.exit()
